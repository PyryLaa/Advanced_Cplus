...
void swap1(int &a_r,int& b_r); //a_r and b_r are reference variables to the values passed to the function
void swap2(int *c_p, int *d_p); //c_p and d_p are pointer variables to the variables passed to the function
...
void swap1(int &a_r, int &b_r) { //Swaps the numbers passed to the function. Numbers are passed as their original variables but handled as references in the function
	int temp;
	temp = a_r;
	a_r = b_r;
	b_r = temp;
...
void swap2(int* c_p, int* d_p) { //Swaps the numbers passed to the function as pointers to the original variables and returns the changed values as pointers
	int temp;
	temp = *c_p;
	*c_p = *d_p;
	*d_p = temp;
